...
export function promise(type: string = '', action: ?string) {
  ...
  if (action) {
    ...
    const create = request.includes('C')
      ? [`CREATE_${str}`, `CREATE_${str}_SUCCESS`, `CREATE_${str}_FAILURE`]
      : [];

    const load = request.includes('L')
      ? [`LOAD_${str}`, `LOAD_${str}_SUCCESS`, `LOAD_${str}_FAILURE`]
      : [];

    const update = request.includes('U')
      ? [`UPDATE_${str}`, `UPDATE_${str}_SUCCESS`, `UPDATE_${str}_FAILURE`]
      : [];

    const del = request.includes('D')
      ? [`DELETE_${str}`, `DELETE_${str}_SUCCESS`, `DELETE_${str}_FAILURE`]
      : [];
